...
# detect_abbr = re.compile(r"((( |^)(([a-zA-Z]+\.( |$)+){2,}|([Pp](ag)?|[Cc]h|[Tt]h|[Vv](ol)?|ib)\.?( |$)+)+([0-9]+\.?,? *)*)+|i\. +e\.)") # any sequence of 2 or more words ending in a period or one of a short list of section abbreviaions (e.g. page, chapter, etc.) optionally followed by a number  
            ...
            for word in revisedline:
                ...
                if re.search("###$",word):
                   if re.search("p(ag)?\.? +([0-9]+,? *)+\. *###",word): # look for page numbers and remove marker
                    ...
                    currentline.append(word)
                if re.search("\.$",word):
                    if not (check_abbr(abbrlist,word) or check_abbr(abbrlist,word[:-1])):
...
#                if re.search("###$",word):
#                    currentline.append(word[:-3].strip())
#                    if re.search("p(ag)? +([0-9],?)+\.?###",word):
#                        sentences.append(currentline)
#                        currentline = []
#                        print word
#                else:
#                    currentline.append(word.strip())
#                if re.search(".*\.$",word):
#                    if not check_abbr(abbrlist,word) and not check_abbr(abbrlist,word[:-1]) and not re.match("(etc\.?|in|et)",word):
##                if not ((len(currentline) == 2) and re.match("[0-9]+",currentline[0]) and (currentline[1] == "."))or not ((len(currentline) == 1) and (currentline == "e.")): #ignore sentences of length 2 in the format # . as they are probably part of a list
#                        sentences.append(currentline)
#                        currentline = []
#    return sentences
